...
for j = 1:n_f
    ...
    CEUS_ini = CEUS_ini + 0.03 + rand(128,128)*0.08; 
    %CEUS_right = zeros(128,128);
    %CEUS_right = CEUS_right + 0.03 + rand(128,128)*0.08;
    %CEUS_left = zeros(128,128);
    %CEUS_left = CEUS_left + 0.03 + rand(128,128)*0.08;
    %CEUS_up = zeros(128,128);
    %CEUS_up = CEUS_up + 0.03 + rand(128,128)*0.08;
    %CEUS_down = zeros(128,128);
    %CEUS_down = CEUS_down + 0.03 + rand(128,128)*0.08;
    ...
    y0 = (rand(1,n_MB)*2-1)*50;
        ...
        sigmax = (rand(1,n_MB)*1+0.5)*2;  %sigma x, sigma y, shape of MB on x,y axis
        sigmay = (rand(1,n_MB)*1+0.5)*2;  %range:(0.9,3.15) to (1,3)
    ...
    filepath=pwd;
    cd('D:\IC\Biomedical\Individual Project\Dataset\MultiMB2\CEUS');
    imwrite(CEUS_ini,filename); % output the image and name
    cd(filepath);
    
    %filename = sprintf ('CEUS%d.jpg',j_right); 
    %filepath=pwd;
    %cd('D:\IC\Biomedical\Individual Project\Dataset\MultiMBTS2\CEUS');
    ...
    for i = 1:n_MB
        sigmax_r(i) = double(int16(sigmax(i).*5)); %4.5-15.75 to 5-15
        sigmay_r(i) = double(int16(sigmay(i).*5));
